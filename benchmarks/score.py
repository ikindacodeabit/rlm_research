...
def aggregate(results_dir: str):
    ...
    for path in sorted(Path(results_dir).rglob("*.jsonl")):
        ...
        for line in open(path):
            ...
            row = rows[key]
            row["n"] += 1
            # prefer the partial-recall float; old records only have boolean correct
            row["score"] += float(r["score"]) if r.get("score") is not None else bool(r.get("correct"))
...
def main() -> None:
    ...
    hdr = (f"{'task':<14}{'subset':<16}{'variant':<16}{'mode':<9}{'model':<22}"
           f"{'budget':>8}{'n':>5}{'acc%':>8}{'tok/q':>9}{'s/q':>7}"
           f"{'peakctx':>9}{'unfin':>7}{'err':>5}")
    print(hdr)
    print("-" * len(hdr))
    csv_rows = []
    for (task, variant, mode, model, subset), r in sorted(rows.items()):
        n = r["n"] or 1
        ...
        tok_q = r["tokens"] // n
        s_q = r["latency"] / n
        print(f"{task:<14}{subset:<16}{variant:<16}{mode:<9}{model:<22}"
        ...
        csv_rows.append({
            ...
            "model": model, "budget": budget, "n": r["n"], "acc": round(acc, 1),
    ...
    if args.csv:
        fields = ["task", "subset", "variant", "mode", "model", "budget", "n", "acc",
